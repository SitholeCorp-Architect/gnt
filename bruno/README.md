...
tools), the generic webhook, gaps/ROI, org offboarding, and apps/store's
...
accept a live Better Auth session JWT (minted by whatever frontend handles sign-in), closing
the self-escalation path an API key minting another, more capable key would otherwise open.
...
A safe read-only subset — no LLM cost, no webhook/store side effects, safe to run
unattended against a real test org on your own deploy:
...
when it was built (auth, rules, the published MCP endpoint, webhooks, gaps/ROI,
