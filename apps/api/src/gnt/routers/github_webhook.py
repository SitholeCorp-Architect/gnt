...
async def github_webhook(
    ...
    # Neither carries a top-level `repository` key the way every other
    # webhook this endpoint handles does (installation events are
    # installation-wide, not repo-scoped), so these are dispatched before
    # the repository lookup below rather than trying to force them through
    # it. See _handle_installation_lifecycle_event's own docstring for why
    # this exists at all.
    if event in ("installation", "installation_repositories"):
        return await _handle_installation_lifecycle_event(session, event, payload, raw_body, signature)
...
async def _handle_installation_lifecycle_event(
    session: AsyncSession, event: str, payload: dict, raw_body: bytes, signature: str | None
) -> JSONResponse:
    """`installation` (action="deleted") and `installation_repositories`
    (action="removed") are the two GitHub-side events that can leave
    github_connections stale: someone uninstalling the App, or narrowing
    its repo access, directly from GitHub's own UI instead of through
    disconnect_github/select_repos (routers/github.py) here. Without this,
    gnt's DB kept reporting those repos as connected long after GitHub
    itself revoked access -- hasEnabledGithubConnectionForWeb (apps/web's
    onboarding-wizard.ts) would report the connect step done off a row
    GitHub had already killed, and the first real call against it (a
    scan, a rule proposal) is what surfaced the failure instead of the
    state ever getting corrected. Reconnecting afterward still worked at
    the DB level (the App callback's own upsert doesn't care what was
    there before), but the wizard never gave the user a chance to,
    because it never stopped believing connect was already done.

    Always App-flow -- a PAT connection has no installation behind it to
    receive these -- so this signs with the shared App secret directly,
    the same one routers/github_webhook.py's main handler falls back to
    for any App-connected connection, rather than looking up a specific
    connection's own secret first.
    """
    installation_id = (payload.get("installation") or {}).get("id")
    if not isinstance(installation_id, int):
        return JSONResponse({"error": "missing installation"}, status_code=400)

    app_secret = get_settings().github_app_webhook_secret
    if not app_secret or not _verify_signature(app_secret, raw_body, signature):
        return JSONResponse({"error": "invalid signature"}, status_code=401)
    ...
    if event == "installation" and action == "deleted":
        connections = (
            await session.execute(
                select(GithubConnection).where(GithubConnection.installation_id == installation_id)
            )
        ).scalars().all()
        for connection in connections:
            await session.delete(connection)
        await session.commit()
        return JSONResponse({"ok": True, "removed_count": len(connections)})

    if event == "installation_repositories" and action == "removed":
        removed_full_names = {r["full_name"] for r in payload.get("repositories_removed", []) if r.get("full_name")}
        removed_urls = {f"https://github.com/{name}" for name in removed_full_names}
        connections = (
            await session.execute(
                select(GithubConnection).where(
                    GithubConnection.installation_id == installation_id,
                    GithubConnection.repo_url.in_(removed_urls),
                )
            )
        ).scalars().all()
        for connection in connections:
    ...
    # Every other action (installation "created"/"new_permissions_accepted"/
    # "suspend"/"unsuspend", installation_repositories "added") doesn't
    # leave a stale row behind -- nothing to reconcile.
    return JSONResponse({"ok": True})
