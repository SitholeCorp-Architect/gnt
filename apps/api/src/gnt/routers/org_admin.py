...
async def confirm_offboarding(
    ...
    # Every row this org has (not just the one get_primary_connection would
    # pick) -- an App install can grant several repos now (see
    # GithubConnection's own docstring), and this is the one place that
    # genuinely needs to know about all of them, not just the one gnt
    # currently proposes rules against. They all share one installation_id,
    # so uninstalling it once revokes access to every one of them.
    connections = (
