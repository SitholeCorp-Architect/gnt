...
from gnt.db.models import McpApiKey, OnboardingEvent, Org, SkillPack, SlackConnection
from gnt.db.session import get_session
from gnt.github.app_auth import get_primary_connection
...
async def onboarding_status(
    ...
    # Same default-true-for-pre-existing-orgs posture as
    # isWizardCompleteForWeb on the web side (migration 0036) -- an org
    # with no row here at all shouldn't be possible past get_current_org,
    # but defaulting true rather than false keeps this from ever wrongly
    # telling a real, working org's CLI it still isn't set up.
    onboarding_completed = (
        await session.execute(select(Org.onboarding_completed).where(Org.id == org.org_id))
    ).scalar_one_or_none()
    ...
    github_connection = await get_primary_connection(session, org.org_id)
