...
async def get_primary_connection(session: AsyncSession, org_id: str) -> GithubConnection | None:
    """The one connection to act against for every call site that only
    ever needed a single repo (propose a rule, the nightly staleness/
    contradiction sweeps, the GitHub-upgrade nudge in routers/brain.py) —
    unchanged behavior for a PAT-connected org (still exactly one row) or
    an App-connected org enabled on a single repo. For an org enabled on
    several repos, picks the oldest enabled connection, ignoring any it has
    turned off.

    ponytail: single-repo target for all of these — real per-repo rule
    routing for a multi-repo org is future work (see GithubConnection's own
    docstring), not required for onboarding to let someone connect more
    than one repo.
    """
    return (
        await session.execute(
            select(GithubConnection)
            .where(GithubConnection.org_id == org_id, GithubConnection.enabled.is_(True))
...
class InstallState:
    org_id: str
    user_id: str
    origin: str  # "web" | "cli" | "onboarding" — which finish page the callback should point at
