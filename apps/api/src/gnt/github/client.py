...
async def list_repo_tree(repo_url: str, pat: str, ref: str) -> list[str]:
    """Every file path in the repo at `ref` (a branch name works directly as
    the tree sha -- GitHub resolves it), via one recursive Trees API call.
    Used by the onboarding scan (routers/onboarding.py) to find AI rules
    files anywhere in the repo, not just at its root.

    ponytail: a `truncated: true` response (GitHub's own cap on very large
    trees) just means this returns whatever it got rather than paging
    through the rest -- acceptable for finding a handful of well-known
    filenames (CLAUDE.md, .cursorrules, ...), which are overwhelmingly
    likely to already be present in what GitHub does return, and a real
    gap only for exceptionally large repos. Full pagination is real
    ingestion-v2 work, not required for onboarding's own scan."""
    owner, repo = parse_repo_url(repo_url)
    response = await _call(
        "GET",
        f"{_API_BASE}/repos/{owner}/{repo}/git/trees/{quote(ref)}",
        pat,
        f"list the file tree at {ref}",
        params={"recursive": "1"},
    )
    if response.status_code != 200:
        raise GithubClientError(
            f"could not list the file tree on {owner}/{repo} ({response.status_code})",
            status_code=response.status_code,
        )
    try:
        ...
        raise GithubClientError(f"GitHub returned an invalid tree response for {owner}/{repo}") from exc
