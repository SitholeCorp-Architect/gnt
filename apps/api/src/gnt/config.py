...
class Settings(BaseSettings):
    ...
    # Free-trial abuse scoring (gnt.trial_risk, trial_signals table) —
    # weighted composite score per signup, combining a reused signup IP,
    # device fingerprint, and (once Stripe checkout completes) card
    # fingerprint against OTHER orgs' recent signup signals. Weights are
    # deliberately unequal: an IP can legitimately be shared by two real
    # people (office/household wifi — the false-positive that killed
    # IP-only blocking), a device fingerprint survives until someone
    # switches browsers, but a card fingerprint (Stripe's own, stable
    # across tokenization attempts for the same physical card) is the
    # hardest and most costly signal to fake, so it's weighted heaviest.
    # Founder-tunable numeric knobs, cheap defaults for the validation
    # phase — same convention as contradiction_sweep_max_comparisons_per_org
    # above, not a claim these are the right numbers at scale.
    trial_risk_ip_reuse_weight: int = 20
    trial_risk_device_fingerprint_reuse_weight: int = 40
    trial_risk_card_fingerprint_reuse_weight: int = 100
    # How far back a prior signup's signals still count as "reused" — a
    # device fingerprint seen once, months ago, on an org that's since
    # converted or churned isn't the same signal as one seen an hour ago.
    trial_risk_lookback_days: int = 30
    # Score bands gnt.trial_risk._status_for_score checks the composite
    # score against. Below flag_threshold: clean, passes silently. At or
    # above it but below block_threshold: flagged — written to
    # trial_signals.risk_status for a human to query and review (same
    # "log it, don't auto-act on it" posture gap_tracking.py's log_gap
    # already applies), the trial itself proceeds unchanged. At or above
    # block_threshold: blocked — the one band that actually stops
    # something. With the weights above, IP+device reused together caps at
    # 60 (still flagged, not blocked) — a signup can never be auto-blocked
    # before Stripe checkout supplies a card fingerprint too; see this
    # feature's PR description for why that's left as an open question
    # rather than decided here.
    trial_risk_flag_threshold: int = 50
    trial_risk_block_threshold: int = 150
    ...
    # GitLab connector — a genuine confidential-client OAuth 2.0
    # authorization-code flow (gitlab.com/oauth/authorize + /oauth/token),
    # unlike Linear's PKCE-only public app: GitLab issues a real
    # client_secret, so gnt/gitlab/oauth.py sends one, same shape as
    # notion_client_id/secret above. All optional, same "unset = this
    # integration isn't live yet" convention as github_app_id/
    # github_app_private_key above, not linear_client_id's required-string
    # convention — there's no real GitLab OAuth App to point at yet, so CI/
    # local dev don't need one just to import this module. Every call site
    # that actually builds an authorize URL or exchanges a code fails loud
    # (GitlabNotConfiguredError), not silently, if these are unset when
    # genuinely needed — see gnt/gitlab/oauth.py's is_configured/
    # _require_configured. state secret and token encryption key are
    # generated locally, same convention as every other connector's.
    # gitlab_webhook_secret is the shared secret token GitLab sends back in
    # the X-Gitlab-Token header on every webhook delivery (a plain
    # equality check, not an HMAC signature like GitHub's — see
    # routers/gitlab_webhook.py) — one secret for the whole deploy, same
    # "one App, one shared secret" shape as github_app_webhook_secret.
    gitlab_client_id: str | None = None
    gitlab_client_secret: str | None = None
    gitlab_state_secret: str | None = None
    gitlab_token_encryption_key: str | None = None
