...
- delete_org_postgres_data: hard-deletes every row in every table that
  ...
  (this function had silently drifted behind that grep once before: the
  connector sync/history tables below — Notion, Linear, Intercom,
  Zendesk — plus llm_usage and staleness_refresh_proposals all shipped
  with their own `org_id: Mapped` column and were never added here,
  so an org that offboarded kept real rows in every one of them
  forever. conflicts/interview_questions/knowledge_units have the same
  gap for a different reason: they're the fully-retired knowledge_units
  pipeline (see db/models.py's own comment on it) with no code path
  left that writes to them — belongs in a migration dropping them
  outright, not a fourth entry pattern here, since there's no live
  feature left to leave data behind for. Not done in this change; still
  open. Deliberately includes the legacy `rules`/
  `rule_audit_log` tables even though nothing writes to them anymore
  now that apps/store is the system of record (see Rule's docstring)
  and `ingest_events` even though its only writers were retired:
  pre-migration orgs can have real historical rows sitting in either,
  and leaving them behind after "offboarding" would be exactly the
  kind of data-leak-after-deletion this function exists to prevent.
  trial_signals is the one real org_id column deliberately NOT deleted
  here — see that model's own docstring for why.
...
async def delete_org_postgres_data(session: AsyncSession, org_id: str) -> dict[str, int]:
    ...
    counts["notion_connections"] = (
        await session.execute(delete(NotionConnection).where(NotionConnection.org_id == org_id))
    ).rowcount or 0
    counts["linear_connections"] = (
        await session.execute(delete(LinearConnection).where(LinearConnection.org_id == org_id))
    ).rowcount or 0
    counts["intercom_connections"] = (
        await session.execute(delete(IntercomConnection).where(IntercomConnection.org_id == org_id))
    ).rowcount or 0
    counts["intercom_sync_states"] = (
        await session.execute(delete(IntercomSyncState).where(IntercomSyncState.org_id == org_id))
    ).rowcount or 0
    counts["intercom_processed_items"] = (
        await session.execute(delete(IntercomProcessedItem).where(IntercomProcessedItem.org_id == org_id))
    ).rowcount or 0
    counts["zendesk_connections"] = (
        await session.execute(delete(ZendeskConnection).where(ZendeskConnection.org_id == org_id))
    ).rowcount or 0
    counts["zendesk_sync_states"] = (
        await session.execute(delete(ZendeskSyncState).where(ZendeskSyncState.org_id == org_id))
    ).rowcount or 0
    counts["zendesk_processed_items"] = (
        await session.execute(delete(ZendeskProcessedItem).where(ZendeskProcessedItem.org_id == org_id))
    ).rowcount or 0
    counts["llm_usage"] = (
        await session.execute(delete(LlmUsage).where(LlmUsage.org_id == org_id))
    ).rowcount or 0
    counts["staleness_refresh_proposals"] = (
        await session.execute(
