...
from sqlalchemy import ARRAY, BigInteger, Date, DateTime, ForeignKey, Index, Text, UniqueConstraint, func
...
class Org(Base):
    ...
    # Onboarding wizard's step-3 survey (routers/onboarding.py). Null
    # until the org's admin submits or skips it; survey_completed is what
    # actually distinguishes "skipped" from "hasn't gotten there yet" --
    # see migration 0036 for why that has to be its own column.
    survey_challenges: Mapped[list[str] | None] = mapped_column(ARRAY(Text), nullable=True)
    survey_referral: Mapped[str | None] = mapped_column(nullable=True)
    survey_completed: Mapped[bool] = mapped_column(default=False, server_default="false")
    # Flips true once the wizard's done screen fires (plan task 9). See
    # migration 0036 for why every pre-existing org defaults to true.
    onboarding_completed: Mapped[bool] = mapped_column(default=True, server_default="true")
    # Onboarding wizard's step-2 organization page. name/slug are Better
    # Auth's (org identity); these are pure descriptive metadata -- see
    # migration 0037.
    website: Mapped[str | None] = mapped_column(nullable=True)
    company_size: Mapped[str | None] = mapped_column(nullable=True)
    # Onboarding wizard's step 6-7 (scan + rules import) and step 8
    # (configure) -- see migration 0040 for why each needs its own marker
    # rather than being derived from some other table's row count.
    repo_scan_completed: Mapped[bool] = mapped_column(default=False, server_default="false")
    configure_completed: Mapped[bool] = mapped_column(default=False, server_default="false")
    # Done screen stat tile (plan step 10) -- see migration 0041 for why
    # this is a plain counter rather than derived from the rule store.
    rules_imported_count: Mapped[int] = mapped_column(default=0, server_default="0")
    # Onboarding wizard's step 8 -- see migration 0042 for why this is a
    # JSON blob (the real check_action option list is a fast-follow, this
    # shape is explicitly provisional) rather than dedicated columns.
    check_action_settings: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
...
class GithubConnection(Base):
    """One row per connected rules repo. A PAT-connected org (installation_id
    NULL) still gets exactly one row — org_id stays unique for that case via
    migration 0039's partial index, and routers/github.py's connect_github
    still upserts on org_id the same way it always has. An App-connected org
    (installation_id set) can have one row per repo it granted the GitHub
    App access to — org_id is NOT globally unique anymore, see migration
    0039 for why the old table-wide constraint had to go. `enabled` is what
    lets an org that installed on more repos than it wants gnt active on
    turn specific ones off (routers/onboarding.py's POST /repos) without
    losing the row.

    A dedicated table, not a shared polymorphic Connector — Slack has no
    such abstraction either, and the two connections' shapes (repo_url/
    default_branch vs team_id/bot_user_id) don't overlap enough to justify
    one. pat_encrypted and webhook_secret_encrypted are Fernet ciphertext
    (gnt/github/crypto.py) under their own dedicated key — separate from
    Slack's, since a PAT with write access to a customer's rules repo is a
    materially higher-value secret than a Slack bot token scoped to
    commands/chat/im only. Both NULL for an App-connected row — nothing to
    persist there, since an App-connected org's tokens are minted
    per-operation and never stored, see gnt/github/app_auth.py.
    `gnt connect github --upgrade` swaps an org's one PAT row for however
    many App rows its install grants (same org_id, same repo_url-keyed
    upsert routers/github.py's App callback already uses).

    ponytail: every call site that reads "the org's connected repo" (rule
    propose, the nightly staleness/contradiction sweeps, the GitHub-upgrade
    nudge) still only ever acts against ONE connection — see
    gnt/github/app_auth.py's get_primary_connection. Real per-repo rule
    routing for an org enabled on several repos is ingestion-v2-shaped
    work, not required for onboarding to let someone connect more than one
    repo.
    ...
    # True for every row by default — an App install already went through
    # GitHub's own repo picker, so the repos it grants start active. Flipped
    # off, not deleted, by routers/onboarding.py's POST /repos when an org
    # wants gnt inactive on a repo it granted access to.
    ...
    # repo_url, not through github/crypto.py's Fernet discipline. NOT
    # unique (migration 0039 dropped that) -- one install now legitimately
    # backs several rows, one per repo it was granted.
...
class GitlabConnection(Base):
    """One connected GitLab account per org — same dashboard-acquires OAuth
    shape as LinearConnection (see that model's own docstring for the
    acquire-here/CLI-reads-there split, applies unchanged here), but
    GitLab's is a genuine confidential-client authorization-code flow
    (client_secret, not PKCE) whose access tokens expire in ~2 hours
    (docs.gitlab.com/ee/api/oauth2.html) and come with a refresh_token —
    unlike Linear's/Notion's effectively-long-lived tokens, so this is the
    first connector table that needs to persist a refresh token and an
    expiry alongside the access token.

    project_path is nullable: OAuth here authorizes a GitLab *account*, not
    a specific project, so which repo gnt should read rules from is a
    separate selection this scaffold doesn't build yet (same shape
    GithubConnection's own App-connected rows need a follow-up repo-select
    step for) — it's here now so that follow-up has somewhere to write.
    `enabled` mirrors GithubConnection's own flag for the same reason: a
    place to turn a connected project off without losing the row.

    access_token_encrypted/refresh_token_encrypted are Fernet ciphertext
    (gnt/gitlab/crypto.py) under their own dedicated key, same reasoning as
    every other connector's key in this file."""
    ...
    project_path: Mapped[str | None] = mapped_column(default=None)
...
class TrialSignal(Base):
    """Per-org free-trial abuse signals — one row per org, upserted once at
    signup and again when Stripe's card fingerprint lands, not appended:
    this describes the CURRENT state of one org's onboarding risk, not a
    history of events (same shape RuleStaleness/ZendeskSyncState already
    use for their own "current snapshot" tables).

    A separate table rather than more columns on Org, because its fields
    arrive at two genuinely different points in the real signup flow:
    signup_ip/device_fingerprint are set once, at routers/billing.py's
    checkout() (the earliest point apps/api ever sees a device
    fingerprint — see apps/web's onboarding-billing-client.tsx), while
    card_fingerprint stays null until that same org's Stripe Checkout
    actually completes (routers/billing.py's webhook handler, minutes to
    never after signup, since a visitor can abandon Checkout). Bolting
    all four onto Org would mean either a NOT NULL Org column that's
    genuinely unknown for a while, or scattering "when is this actually
    set" reasoning across Org's own docstring alongside a dozen unrelated
    billing fields — see gnt.trial_risk for the scoring logic that reads
    and writes this table.

    No RLS — same reasoning mcp_api_keys/webhook_tokens give (migration
    0007): scoring THIS org's risk means reading recent signals across
    EVERY org (looking for a reused ip/device_fingerprint/card_fingerprint
    against a prior signup), which an RLS-scoped session can never do — a
    scoped SELECT would only ever see its own org's row, so every signup
    would silently score "clean" no matter what. That's a materially worse
    failure mode than skipping tenant isolation here, since catching
    cross-org reuse is the entire point of this table. Nothing exposes
    these rows back to a customer (no list/detail endpoint reads this
    table), so the missing isolation never surfaces outside gnt.trial_risk's
    own internal scoring reads.

    Deliberately NOT deleted by org_offboarding.py, on purpose, same
    reasoning that module's own docstring gives for leaving orgs.
    trial_ends_at alone: dropping this row on offboarding would let
    someone offboard and re-signup with the same ip/device/card and score
    clean a second time, defeating the reason this table exists."""

    __tablename__ = "trial_signals"

    id: Mapped[uuid.UUID] = _uuid_pk()
    org_id: Mapped[str] = mapped_column(ForeignKey("orgs.id"), unique=True, index=True)
    signup_ip: Mapped[str | None] = mapped_column(nullable=True)
    device_fingerprint: Mapped[str | None] = mapped_column(nullable=True)
    # Null until Stripe checkout completes — see routers/billing.py's
    # webhook handler (apply_webhook_event's checkout.session.completed
    # branch), the only writer.
    card_fingerprint: Mapped[str | None] = mapped_column(nullable=True)
    risk_score: Mapped[int] = mapped_column(default=0, server_default="0")
    # clean|flagged|blocked — gnt.trial_risk's three score bands, see that
    # module's own constants in config.py (trial_risk_flag_threshold/
    # trial_risk_block_threshold) for where the boundaries actually live.
    risk_status: Mapped[str] = mapped_column(default="clean", server_default="clean")
    created_at: Mapped[datetime] = _timestamp()
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
    ...
    __table_args__ = (
        Index("ix_trial_signals_signup_ip", "signup_ip"),
        Index("ix_trial_signals_device_fingerprint", "device_fingerprint"),
        Index("ix_trial_signals_card_fingerprint", "card_fingerprint"),
    )
