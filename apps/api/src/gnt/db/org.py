...
async def ensure_org(session: AsyncSession, org_id: str) -> None:
    ...
    stmt = (
        insert(Org)
        # Every genuinely new org starts the wizard over -- the column's
        # own server_default covers pre-existing rows (migration 0036),
        # this covers the one path that creates new ones.
        .values(id=org_id, trial_ends_at=trial_ends_at, onboarding_completed=False)
