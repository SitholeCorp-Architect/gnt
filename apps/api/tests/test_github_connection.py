...
async def test_disconnect_removes_every_row_and_uninstalls_once_for_a_multi_repo_org(
    admin_session, db_session, org_a, monkeypatch
):
    """An org enabled on several repos from one App install has several
    rows sharing one installation_id -- disconnect must delete all of them
    and only call uninstall_app once (they all point at the same
    installation), not once per row."""
    from gnt.db.org import ensure_org
...
async def test_callback_persists_the_connection_and_swaps_a_pat_row_over_on_upgrade(
    ...
    async with admin_session as client:
        ...
        assert callback.json() == {
            "ok": True,
            "origin": "cli",
            "repo_urls": ["https://github.com/acme/rules"],
        }
...
async def test_callback_persists_every_granted_repo(admin_session, db_session, org_a, app_configured, monkeypatch):
    """The old callback hard-rejected an install granted more than one
    repo -- that was always a placeholder (GitHub's own install picker
    already lets someone grant several), so this asserts the real
    behavior: one row per repo, all enabled, all sharing the installation
    that granted them."""
    from gnt.github.app_auth import build_install_state
    ...
    async with admin_session as client:
        state = build_install_state(org_a, "admin_a", origin="onboarding")
        r = await client.get(
            "/v1/settings/github/app/callback",
            params={"installation_id": 555, "setup_action": "install", "state": state},
        )
        ...
        assert sorted(body["repo_urls"]) == ["https://github.com/acme/other", "https://github.com/acme/rules"]

    rows = (
        await db_session.execute(select(GithubConnection).where(GithubConnection.org_id == org_a))
    ).scalars().all()
    assert len(rows) == 2
    assert {row.repo_url for row in rows} == {"https://github.com/acme/rules", "https://github.com/acme/other"}
    assert all(row.installation_id == 555 for row in rows)
    assert all(row.enabled is True for row in rows)


async def test_callback_rejects_an_install_with_zero_granted_repos(
    admin_session, db_session, org_a, app_configured, monkeypatch
):
    from gnt.github.app_auth import build_install_state
