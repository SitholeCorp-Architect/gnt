...
async def db_session(monkeypatch) -> AsyncIterator[AsyncSession]:
    ...
    Also patches gnt.mcp_server.server's, gnt.mcp_server.auth's, and
    gnt.llm_quota's get_sessionmaker so MCP tool functions, McpAuthMiddleware,
    and the per-org LLM spend quota gate (all three open their own session
