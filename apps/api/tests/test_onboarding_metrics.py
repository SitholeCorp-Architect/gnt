...
async def test_onboarding_status_is_scoped_to_current_org(db_session, test_app_factory, org_a, org_b):
    ...
    # log_onboarding_event above re-scopes app.current_org to org_a as
    # part of its own commit (see its own docstring) -- make_org_client's
    # dependency override bypasses get_current_org entirely (that's the
    # whole point, no real JWT/API key needed), which is also what
    # normally re-scopes to whichever org a request is actually for
    # (auth/better_auth.py's get_current_org calls scope_to_org before
    # returning, every real request). Without this, the GUC is still
    # left on org_a when this simulates a request as org_b, and the
    # orgs table's own forced RLS policy (unlike the other org-scoped
    # tables below) would silently hide org_b's own row from this
    # endpoint's onboarding_completed read.
    await scope_to_org(db_session, org_b)
