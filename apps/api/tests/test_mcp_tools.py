...
def _no_llm_quota_gate(monkeypatch):
    ...
    (gnt.llm_quota, with its own dedicated coverage in
    tests/test_llm_quota.py) and a plan action cap
    (gnt.entitlements.enforce_plan_action_cap — a no-op in this self-host
    build). None of these check_action tests are testing either gate
    (they're testing MCP-tool wiring, tenant isolation, gap/ROI bumping),
    so stub both out here rather than per-test, the same shape
