...
The real quota gate (gnt.llm_quota) and the plan action cap
(gnt.entitlements.enforce_plan_action_cap — a no-op in this self-host
build, never actually raises) are both stubbed out by default for every
test in this file (see _no_llm_quota_gate below) — gnt.llm_quota is a real
Postgres-backed check with its own dedicated coverage
(tests/test_llm_quota.py), and this file's whole point is testing
evaluate_action's verdict logic in isolation, not a live DB. A couple of
tests below override a stub to prove that gate's wiring specifically."""
...
from gnt.entitlements import PlanActionCapExceededError
