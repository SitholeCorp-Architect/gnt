...
# -- installation lifecycle events: GitHub telling gnt directly that the
# App was uninstalled, or that its repo access was narrowed, from GitHub's
# own UI rather than through disconnect_github/select_repos here. See
# github_webhook.py's _handle_installation_lifecycle_event for the full
# reasoning on why leaving these unhandled meant a stale, already-revoked
# connection kept reporting as connected.
# ---------------------------------------------------------------------


def _installation_deleted_payload(installation_id: int) -> dict:
    return {"action": "deleted", "installation": {"id": installation_id}}


def _installation_repositories_removed_payload(installation_id: int, *removed_full_names: str) -> dict:
    return {
        "action": "removed",
        "installation": {"id": installation_id},
        "repositories_removed": [{"full_name": name} for name in removed_full_names],
    }


async def test_installation_deleted_removes_every_connection_for_that_installation(
    webhook_client, db_session, org_a, app_webhook_secret
):
    await _connect_github_app(db_session, org_a, installation_id=111, repo_url=APP_REPO_URL)
    await _connect_github_app(db_session, org_a, installation_id=111, repo_url="https://github.com/acme/app-rules-2")

    body = json.dumps(_installation_deleted_payload(111)).encode("utf-8")
    async with webhook_client as client:
        res = await client.post(
            "/v1/github/webhook",
            content=body,
    ...
    assert res.status_code == 200
    assert res.json() == {"ok": True, "removed_count": 2}

    remaining = (
        await db_session.execute(select(GithubConnection).where(GithubConnection.installation_id == 111))
    ).scalars().all()
    assert remaining == []


async def test_installation_deleted_does_not_touch_a_different_installation(
    webhook_client, db_session, org_a, org_b, app_webhook_secret
):
    await _connect_github_app(db_session, org_a, installation_id=111, repo_url=APP_REPO_URL)
    await _connect_github_app(db_session, org_b, installation_id=222, repo_url=OTHER_APP_REPO_URL)
    ...
    still_there = (
        await db_session.execute(select(GithubConnection).where(GithubConnection.installation_id == 222))
    ).scalar_one_or_none()
    assert still_there is not None


async def test_installation_repositories_removed_only_deletes_the_named_repos(
    webhook_client, db_session, org_a, app_webhook_secret
):
    kept_url = "https://github.com/acme/app-rules-kept"
    await _connect_github_app(db_session, org_a, installation_id=111, repo_url=APP_REPO_URL)
    await _connect_github_app(db_session, org_a, installation_id=111, repo_url=kept_url)

    body = json.dumps(_installation_repositories_removed_payload(111, "acme/app-rules")).encode("utf-8")
    async with webhook_client as client:
        res = await client.post(
            "/v1/github/webhook",
            content=body,
    ...
    assert res.status_code == 200
    assert res.json() == {"ok": True, "removed_count": 1}

    removed = (
        await db_session.execute(select(GithubConnection).where(GithubConnection.repo_url == APP_REPO_URL))
    ).scalar_one_or_none()
    assert removed is None
    kept = (
        await db_session.execute(select(GithubConnection).where(GithubConnection.repo_url == kept_url))
    ).scalar_one_or_none()
    assert kept is not None


async def test_installation_deleted_with_a_forged_signature_401s_without_deleting_anything(
    webhook_client, db_session, org_a, app_webhook_secret
):
    await _connect_github_app(db_session, org_a, installation_id=111, repo_url=APP_REPO_URL)
...
async def test_installation_created_is_a_clean_no_op(webhook_client, db_session, org_a, app_webhook_secret):
    """No stale row to reconcile on install -- routers/github.py's own
    /app/callback is what creates the connection, this event is
    informational only. Confirms the dispatch doesn't mistake "created"
    for "deleted" and delete something that was never stale."""
    await _connect_github_app(db_session, org_a, installation_id=111, repo_url=APP_REPO_URL)

    body = json.dumps({"action": "created", "installation": {"id": 111}}).encode("utf-8")
    async with webhook_client as client:
        res = await client.post(
            "/v1/github/webhook",
            content=body,
