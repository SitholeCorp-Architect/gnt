...
// Same field lib/onboarding-wizard.ts's isWizardCompleteForWeb and
// GET /v1/onboarding/status both read off orgs.onboarding_completed --
// the one real "is this org set up" answer now (see brain.py's own
// docstring on this endpoint). gnt login no longer assumes "key minted"
// means "set up" the way it implicitly used to (that was this command's
// own onboarding proxy, same one lib/onboarding-complete.ts's web-side
// version got retired for) -- it asks the same real question the
// dashboard does instead.
export async function fetchOnboardingCompleted(key: string): Promise<boolean | null> {
  try {
    ...
    return typeof body.onboarding_completed === "boolean" ? body.onboarding_completed : null;
  } catch {
    return null;
  }
}
...
export async function login(): Promise<void> {
  ...
  try {
    ...
    // Best-effort -- a network hiccup here must not make an otherwise-
    // successful login look like it failed. onboarding_completed is null
    // (not false) when this can't be determined, so it gets its own
    // branch rather than folding into the "not done yet" message.
    const onboardingCompleted = await fetchOnboardingCompleted(key);
    if (onboardingCompleted === true) {
      wait.stop(
        `${ok("Logged in.")} ${text("✓ set up")} — ${dim(`${WEB_URL}/welcome`)}`,
      );
    } else if (onboardingCompleted === false) {
      // The org exists (the key mint above needs one) but hasn't finished
      // the browser wizard -- same one every website signup goes through
      // (see cli-login-client.tsx's own comment), not something this
      // command tries to walk through itself anymore.
      wait.stop(
        `${ok("Logged in.")} ${text("Setup isn't finished yet")} — finish it at ${dim(`${WEB_URL}/onboarding`)}`,
      );
    } else {
