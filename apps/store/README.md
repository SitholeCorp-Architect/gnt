...
that. This service is never supposed to have either attached; whatever host
runs `apps/api` should reach it over a private network, not a public one
— see `STORE_API_URL` in `docs/self-hosting/env-vars.md`.
